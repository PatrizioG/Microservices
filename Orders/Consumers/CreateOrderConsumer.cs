...
public class CreateOrderConsumer : IConsumer<CreateOrder>
{
    ...
    public async Task Consume(ConsumeContext<CreateOrder> context)
    {
        ...
        foreach (var orderLine in context.Message.OrderLines)
        {
            ...
            var response2 =
                await _productRequestClient.GetResponse<Common.Contracts.Product>(new Common.Contracts.GetProductById
        ...
        await context.RespondAsync<Order>(OrderMapper.MapOrder(orderEntity, products.Select(x => x.Item1).ToList(),
            user));
    }
}